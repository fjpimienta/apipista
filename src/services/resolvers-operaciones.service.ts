...
class ResolversOperationsService {
  ...
  protected async list(
    ...
    sort: Record<string, 1 | -1> = { id: 1 }
  ) {
    try {
      sort = { ...sort };
      const paginationData = await pagination(this.getDB(), collection, page, itemsPage, filter);
      const items = await findElements(this.getDB(), collection, filter, paginationData, sort);
    ...
    try {
      return await insertOneElement(this.getDB(), collection, document).then(
        res => {
          ...
          if (res.acknowledged) {  // Cambiar 'res.result.ok' por 'res.acknowledged'
            return {
              status: true,
    ...
    try {
      return await updateOneElement(
        ...
        res => {
          ...
          // Cambiar 'res.result.nModified' por 'res.modifiedCount' y 'res.result.ok' por 'res.acknowledged'
          ...
          return {
            status: false,
            message: `El registro de ${item} no se ha actualizado. Comprueba que estás filtrando correctamente. O simplemente no hay nada que actualizar.`,
            item: null
          };
        });
    } catch (error) {
