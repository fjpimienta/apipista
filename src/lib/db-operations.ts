...
import { IPaginationOptions } from '../interfaces/pagination-options.interface.js';
...
export const asignDocumentId = async (
  ...
  sort: Record<string, 1 | -1> = { registerDate: -1 }
) => {
...
export const manageStockUpdate = async (
  ...
  collection: string,
  filter: object,
  updateObject: Record<string, number> // Especificar que updateObject es un objeto con claves de tipo string y valores numéricos
) => {
  return await database.collection(collection).updateOne(
    filter,
    { $inc: updateObject }
